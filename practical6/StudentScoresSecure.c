...
Secure array operations with validation
@Nabina Rana */
...
//Secure function to read a single score
bool readSingleScore(int *score) {
...
//secure function to read all scores
bool readScores(int scores[], int size){
    if (!validateArrayParams(scores, size)) {
        return false;
    }

    printf("Enter %d students scores (0-100):\n", size);
    ...
    for (int i = 0; i < size; i++) {
        ...
        while (attempts < MAX_ATTEMPTS && !validInput) {
            ...
            if (!readSingleScore(&scores[i])) {
                attempts++;
                printf("Invalid input. Attempts remaining: %d\n",
